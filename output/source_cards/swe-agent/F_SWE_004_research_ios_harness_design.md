# Source Card: F_SWE_004 - SWE-agent ACI 到 iOS App Store Harness 的迁移设计研究报告
...
| source_id | F_SWE_004 |
| framework | swe-agent |
| raw_path | raw/Kimi_Agent_SWE-agent 迁移研究/research/ios_harness_design.md |
| file_type | markdown |
| topic | SWE-agent ACI 到 iOS App Store Harness 的迁移设计研究报告 |
...
- 它围绕 `SWE-agent ACI 到 iOS App Store Harness 的迁移设计研究报告` 展开，主要贡献 ACI、工具、环境反馈、trajectory、workflow 或安全边界。
...
|---|---|---|---|---|
| M-SWE-013 | iOS ACI Tool Suite | 把 iOS Harness 需要的工具拆成 view/search/context/edit/run/test/build/privacy/release/trajectory 等脚本。 | E1 | high |
| M-SWE-014 | Tool Contract Schema | 每个工具需要输入、输出、失败码和安全级别。 | E2 | high |
| M-SWE-015 | Task-to-Tool Routing | 不同 iOS 任务选择不同工具组合。 | E3 | high |
| M-SWE-016 | ACI Version Gate | v0.1 保留文档规则，v0.5 引入脚本，v1.0 才做 runtime 拦截。 | E4 | high |
...
|---|---|---|---|
| E1 | 主标题/首个 heading 指向：SWE-agent ACI 到 iOS App Store Harness 的迁移设计研究报告。 | raw/Kimi_Agent_SWE-agent 迁移研究/research/ios_harness_design.md:1 | source topic |
| E2 | 关键结构摘要：SWE-agent ACI 到 iOS App Store Harness 的迁移设计研究报告; 目录; 第一部分：Flutter 项目结构和工作流; 1.1 标准项目目录树; 1.2 关键文件清单; 1.3 常用开发命令。 | raw/Kimi_Agent_SWE-agent 迁移研究/research/ios_harness_design.md:structure | mechanism structure |
| E3 | inventory 主题为：SWE-agent ACI 到 iOS App Store Harness 的迁移设计研究报告。 | SOURCE_INVENTORY.md | estimated topic |
| E4 | 文件类别 `markdown`，度量值 1917，细节：markdown headings。 | raw/Kimi_Agent_SWE-agent 迁移研究/research/ios_harness_design.md:full file | scope and density |
| E5 | 该文件归类为 SWE-agent `ios_aci_design` 清洗资料。 | raw/Kimi_Agent_SWE-agent 迁移研究/research/ios_harness_design.md:path | framework category |
...
|---|---|---|---|---|
| M-SWE-013 | Action / ACI Layer | scripts/agent/ | v0_5 | 形成 iOS ACI 工具目录。 |
| M-SWE-014 | Action / ACI Layer | docs/agent/ACI_TOOL_CONTRACTS.md | v0_5 | 为工具调用写契约。 |
| M-SWE-015 | Task Layer | TASKS.md | v0_5 | 在 task card 中加入 required_tools。 |
| M-SWE-016 | Risk / Release Layer | output/ios_harness_mapping/v0_5_scope.md | v0_5 | 区分版本能力。 |
...
| F_SWE_023 | iOS ACI tools section |

## 13. Clean Summary for Codex

这张卡把 `SWE-agent ACI 到 iOS App Store Harness 的迁移设计研究报告` 从原始 SWE-agent 调研文件转成可被 Codex 消费的 clean asset。后续 iOS Harness 构建时，不应直接读取 raw 文件理解 SWE-agent，而应读取这张 Source Card 及其 framework summary。它的主要价值是把 SWE-agent 的 Agent-Computer Interface、受控工具调用、环境反馈、trajectory/replay、命令验证、安全权限和 iOS 场景 workflow 迁移到 Harness 的 Action、Feedback、Memory 和 Risk 层。合成阶段需要与 Aider 的 repo/file scope、GSD2 的状态机与验证闭环、gstack 的角色/审查治理、Superpowers 的工程纪律融合。
