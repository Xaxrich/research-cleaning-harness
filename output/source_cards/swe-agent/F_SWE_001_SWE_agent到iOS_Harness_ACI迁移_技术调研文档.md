# Source Card: F_SWE_001 - SWE agent到iOS Harness ACI迁移 技术调研文档
...
| source_id | F_SWE_001 |
| framework | swe-agent |
| raw_path | raw/Kimi_Agent_SWE-agent 迁移研究/SWE-agent到iOS_Harness_ACI迁移_技术调研文档.docx |
| file_type | document |
| topic | SWE agent到iOS Harness ACI迁移 技术调研文档 |
...
- 它围绕 `SWE agent到iOS Harness ACI迁移 技术调研文档` 展开，主要贡献 ACI、工具、环境反馈、trajectory、workflow 或安全边界。
...
|---|---|---|---|---|
| M-SWE-001 | Full SWE-to-iOS Migration Report | 完整报告汇总 ACI、工具、trajectory、workflow、安全和交付物。 | E1 | high |
| M-SWE-002 | ACI Runtime Migration Thesis | 报告主张将 SWE-agent 的行动接口迁移为 iOS Harness 的工具层。 | E2 | high |
| M-SWE-003 | Versioned Scope Split | 将 v0.1 文档规则、v0.5 脚本、v1.0 runtime 分开。 | E3 | high |
| M-SWE-004 | Duplicate Report Warning | 完整报告、converted、base 和 footnote 文件可能内容重复。 | E4 | high |
...
|---|---|---|---|
| E1 | 主标题/首个 heading 指向：SWE agent到iOS Harness ACI迁移 技术调研文档。 | raw/Kimi_Agent_SWE-agent 迁移研究/SWE-agent到iOS_Harness_ACI迁移_技术调研文档.docx:1 | source topic |
| E2 | 关键结构摘要：SWE agent到iOS Harness ACI迁移 技术调研文档。 | raw/Kimi_Agent_SWE-agent 迁移研究/SWE-agent到iOS_Harness_ACI迁移_技术调研文档.docx:structure | mechanism structure |
| E3 | inventory 主题为：SWE agent到iOS Harness ACI迁移 技术调研文档。 | SOURCE_INVENTORY.md | estimated topic |
| E4 | 文件类别 `document`，度量值 16，细节：document entries: [Content_Types].xml; docProps/app.xml; docProps/custom.xml; docProps/core.xml; word/fontTable.xml; word/comments.xml; word/numbering.xml; word/footnotes.xml。 | raw/Kimi_Agent_SWE-agent 迁移研究/SWE-agent到iOS_Harness_ACI迁移_技术调研文档.docx:full file | scope and density |
| E5 | 该文件归类为 SWE-agent `report` 清洗资料。 | raw/Kimi_Agent_SWE-agent 迁移研究/SWE-agent到iOS_Harness_ACI迁移_技术调研文档.docx:path | framework category |
...
|---|---|---|---|---|
| M-SWE-001 | Harness Maintenance Layer | output/frameworks/swe_agent_summary.md | v0_5 | 作为 SWE-agent 总结主证据之一。 |
| M-SWE-002 | Action / ACI Layer | scripts/agent/ | v0_5 | 确立 SWE 迁移主方向。 |
| M-SWE-003 | Harness Maintenance Layer | output/ios_harness_mapping/v0_1_scope.md | v0_1 | 避免版本混淆。 |
| M-SWE-004 | Risk / Release Layer | output/conflicts/swe_agent_conflicts.md | v0_1 | 在 summary 中去重。 |
...
这张卡把 `SWE agent到iOS Harness ACI迁移 技术调研文档` 从原始 SWE-agent 调研文件转成可被 Codex 消费的 clean asset。后续 iOS Harness 构建时，不应直接读取 raw 文件理解 SWE-agent，而应读取这张 Source Card 及其 framework summary。它的主要价值是把 SWE-agent 的 Agent-Computer Interface、受控工具调用、环境反馈、trajectory/replay、命令验证、安全权限和 iOS 场景 workflow 迁移到 Harness 的 Action、Feedback、Memory 和 Risk 层。合成阶段需要与 Aider 的 repo/file scope、GSD2 的状态机与验证闭环、gstack 的角色/审查治理、Superpowers 的工程纪律融合。
