# Source Card: F_SWE_006 - SWE-agent & mini-SWE-agent 仓库结构与源码组织深度研究报告
...
| source_id | F_SWE_006 |
| framework | swe-agent |
| raw_path | raw/Kimi_Agent_SWE-agent 迁移研究/research/swe_repo_structure.md |
| file_type | markdown |
| topic | SWE-agent & mini-SWE-agent 仓库结构与源码组织深度研究报告 |
...
- 它围绕 `SWE-agent & mini-SWE-agent 仓库结构与源码组织深度研究报告` 展开，主要贡献 ACI、工具、环境反馈、trajectory、workflow 或安全边界。
...
|---|---|---|---|---|
| M-SWE-021 | SWE Repo Component Map | 将 agent、environment、tools、trajectory、config 等目录拆成组件地图。 | E1 | high |
| M-SWE-022 | Environment Abstraction | 把执行环境与 agent 策略解耦。 | E2 | high |
| M-SWE-023 | Configuration as Runtime Input | 使用配置控制模型、工具、环境和任务参数。 | E3 | high |
| M-SWE-024 | Repository Structure Boundary | 仓库结构只能指导设计，不能证明工具行为正确。 | E4 | high |
...
|---|---|---|---|
| E1 | 主标题/首个 heading 指向：SWE-agent & mini-SWE-agent 仓库结构与源码组织深度研究报告。 | raw/Kimi_Agent_SWE-agent 迁移研究/research/swe_repo_structure.md:1 | source topic |
| E2 | 关键结构摘要：SWE-agent & mini-SWE-agent 仓库结构与源码组织深度研究报告; 目录; 第一部分：SWE-agent 目录结构; 1.1 顶层目录树; 1.2 子目录作用表格; 1.3 关键源码文件表格。 | raw/Kimi_Agent_SWE-agent 迁移研究/research/swe_repo_structure.md:structure | mechanism structure |
| E3 | inventory 主题为：SWE-agent & mini-SWE-agent 仓库结构与源码组织深度研究报告。 | SOURCE_INVENTORY.md | estimated topic |
| E4 | 文件类别 `markdown`，度量值 465，细节：markdown headings。 | raw/Kimi_Agent_SWE-agent 迁移研究/research/swe_repo_structure.md:full file | scope and density |
| E5 | 该文件归类为 SWE-agent `repo_structure` 清洗资料。 | raw/Kimi_Agent_SWE-agent 迁移研究/research/swe_repo_structure.md:path | framework category |
...
|---|---|---|---|---|
| M-SWE-021 | Harness Maintenance Layer | ARCHITECTURE.md | v0_1 | 为 iOS Harness 文件树设计提供结构参考。 |
| M-SWE-022 | Action / ACI Layer | docs/agent/ENVIRONMENT_ABSTRACTION.md | v0_5 | 为 simulator、Flutter、Firebase 命令建立环境层。 |
| M-SWE-023 | Memory / State Layer | CONFIG.md | v0_5 | 把运行参数从 prompt 中移出。 |
| M-SWE-024 | Risk / Release Layer | DECISIONS.md | v0_1 | 要求实现阶段补测试。 |
...
| F_SWE_014 | sectioned repo structure |

## 13. Clean Summary for Codex

这张卡把 `SWE-agent & mini-SWE-agent 仓库结构与源码组织深度研究报告` 从原始 SWE-agent 调研文件转成可被 Codex 消费的 clean asset。后续 iOS Harness 构建时，不应直接读取 raw 文件理解 SWE-agent，而应读取这张 Source Card 及其 framework summary。它的主要价值是把 SWE-agent 的 Agent-Computer Interface、受控工具调用、环境反馈、trajectory/replay、命令验证、安全权限和 iOS 场景 workflow 迁移到 Harness 的 Action、Feedback、Memory 和 Risk 层。合成阶段需要与 Aider 的 repo/file scope、GSD2 的状态机与验证闭环、gstack 的角色/审查治理、Superpowers 的工程纪律融合。
