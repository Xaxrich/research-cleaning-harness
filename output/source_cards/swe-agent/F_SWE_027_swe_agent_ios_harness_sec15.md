# Source Card: F_SWE_027 - 15. 五轮框架组合
...
| source_id | F_SWE_027 |
| framework | swe-agent |
| raw_path | raw/Kimi_Agent_SWE-agent 迁移研究/swe_agent_ios_harness_sec15.md |
| file_type | markdown |
| topic | 15. 五轮框架组合 |
...
- 它围绕 `15. 五轮框架组合` 展开，主要贡献 ACI、工具、环境反馈、trajectory、workflow 或安全边界。
...
|---|---|---|---|---|
| M-SWE-105 | Five-Framework Layering | 把 Superpowers、GSD2、Aider、gstack、SWE-agent 分层，避免职责冲突。 | E1 | high |
| M-SWE-106 | SWE Owns Action Runtime | SWE-agent 主要贡献工具接口、运行环境和 trajectory，而非方法论或角色治理。 | E2 | high |
| M-SWE-107 | Composition Boundary | 明确 Aider 负责 repo context，gstack 负责角色，GSD2 负责状态，SWE 负责工具执行。 | E3 | high |
| M-SWE-108 | Conflict-Driven Synthesis | 跨框架融合要记录重叠和冲突。 | E4 | high |
...
|---|---|---|---|
| E1 | 主标题/首个 heading 指向：15. 五轮框架组合。 | raw/Kimi_Agent_SWE-agent 迁移研究/swe_agent_ios_harness_sec15.md:1 | source topic |
| E2 | 关键结构摘要：15. 五轮框架组合; 15.1 五者分层定位; 15.1.1 五层架构：从接口到协作; 15.1.2 重叠点分析：底层文件操作、测试反馈; 15.1.3 冲突点分析：Token 消耗策略、激活方式; 15.2 文件组织策略。 | raw/Kimi_Agent_SWE-agent 迁移研究/swe_agent_ios_harness_sec15.md:structure | mechanism structure |
| E3 | inventory 主题为：15. 五轮框架组合。 | SOURCE_INVENTORY.md | estimated topic |
| E4 | 文件类别 `markdown`，度量值 448，细节：markdown headings。 | raw/Kimi_Agent_SWE-agent 迁移研究/swe_agent_ios_harness_sec15.md:full file | scope and density |
| E5 | 该文件归类为 SWE-agent `framework_comparison` 清洗资料。 | raw/Kimi_Agent_SWE-agent 迁移研究/swe_agent_ios_harness_sec15.md:path | framework category |
...
|---|---|---|---|---|
| M-SWE-105 | Harness Maintenance Layer | output/frameworks/swe_agent_summary.md | v0_5 | 作为跨框架融合输入。 |
| M-SWE-106 | Action / ACI Layer | scripts/agent/ | v0_5 | 将 SWE 机制放入 ACI 层。 |
| M-SWE-107 | Role / Review Layer | AGENTS.md | v0_5 | 写入框架责任边界。 |
| M-SWE-108 | Harness Maintenance Layer | output/conflicts/swe_agent_conflicts.md | v0_1 | 用冲突台账约束最终机制库。 |
...
这张卡把 `15. 五轮框架组合` 从原始 SWE-agent 调研文件转成可被 Codex 消费的 clean asset。后续 iOS Harness 构建时，不应直接读取 raw 文件理解 SWE-agent，而应读取这张 Source Card 及其 framework summary。它的主要价值是把 SWE-agent 的 Agent-Computer Interface、受控工具调用、环境反馈、trajectory/replay、命令验证、安全权限和 iOS 场景 workflow 迁移到 Harness 的 Action、Feedback、Memory 和 Risk 层。合成阶段需要与 Aider 的 repo/file scope、GSD2 的状态机与验证闭环、gstack 的角色/审查治理、Superpowers 的工程纪律融合。
