# Source Card: F_SWE_005 - SWE-agent Agent-Computer Interface (ACI) 设计机制深度研究报告
...
| source_id | F_SWE_005 |
| framework | swe-agent |
| raw_path | raw/Kimi_Agent_SWE-agent 迁移研究/research/swe_aci_mechanisms.md |
| file_type | markdown |
| topic | SWE-agent Agent-Computer Interface (ACI) 设计机制深度研究报告 |
...
- 它围绕 `SWE-agent Agent-Computer Interface (ACI) 设计机制深度研究报告` 展开，主要贡献 ACI、工具、环境反馈、trajectory、workflow 或安全边界。
...
|---|---|---|---|---|
| M-SWE-017 | Agent-Computer Interface Boundary | ACI 把模型意图限制在可审计、可验证的工具接口内。 | E1 | high |
| M-SWE-018 | Observation After Action | 每次工具动作都返回观察结果，避免模型凭想象继续。 | E2 | high |
| M-SWE-019 | Tool-Level Error Semantics | 工具失败需要结构化错误和下一步建议。 | E3 | high |
| M-SWE-020 | ACI Security Envelope | 高风险工具必须有权限、范围和确认边界。 | E4 | high |
...
|---|---|---|---|
| E1 | 主标题/首个 heading 指向：SWE-agent Agent-Computer Interface (ACI) 设计机制深度研究报告。 | raw/Kimi_Agent_SWE-agent 迁移研究/research/swe_aci_mechanisms.md:1 | source topic |
| E2 | 关键结构摘要：SWE-agent Agent-Computer Interface (ACI) 设计机制深度研究报告; 第一部分：ACI 设计哲学; 1.1 ACI 是什么; 1.2 为什么模型不能直接用人用 CLI; 1.3 ACI 的核心设计原则; 原则 1：命令应简单易懂。 | raw/Kimi_Agent_SWE-agent 迁移研究/research/swe_aci_mechanisms.md:structure | mechanism structure |
| E3 | inventory 主题为：SWE-agent Agent-Computer Interface (ACI) 设计机制深度研究报告。 | SOURCE_INVENTORY.md | estimated topic |
| E4 | 文件类别 `markdown`，度量值 681，细节：markdown headings。 | raw/Kimi_Agent_SWE-agent 迁移研究/research/swe_aci_mechanisms.md:full file | scope and density |
| E5 | 该文件归类为 SWE-agent `aci_core` 清洗资料。 | raw/Kimi_Agent_SWE-agent 迁移研究/research/swe_aci_mechanisms.md:path | framework category |
...
|---|---|---|---|---|
| M-SWE-017 | Action / ACI Layer | docs/agent/ACI_TOOL_CONTRACTS.md | v0_5 | 定义模型与文件系统/命令的交互边界。 |
| M-SWE-018 | Feedback / Verification Layer | output/data/trajectory.jsonl | v0_5 | 记录 action/observation。 |
| M-SWE-019 | Feedback / Verification Layer | FAILURE_LOG.md | v0_5 | 把失败归类写入日志。 |
| M-SWE-020 | Risk / Release Layer | RISK_CONTROL.md | v0_1 | 在工具层标注 allow/ask/deny。 |
...
|---|---|
| F_SWE_016 | ACI section |
| F_SWE_023 | iOS tool design |

## 13. Clean Summary for Codex

这张卡把 `SWE-agent Agent-Computer Interface (ACI) 设计机制深度研究报告` 从原始 SWE-agent 调研文件转成可被 Codex 消费的 clean asset。后续 iOS Harness 构建时，不应直接读取 raw 文件理解 SWE-agent，而应读取这张 Source Card 及其 framework summary。它的主要价值是把 SWE-agent 的 Agent-Computer Interface、受控工具调用、环境反馈、trajectory/replay、命令验证、安全权限和 iOS 场景 workflow 迁移到 Harness 的 Action、Feedback、Memory 和 Risk 层。合成阶段需要与 Aider 的 repo/file scope、GSD2 的状态机与验证闭环、gstack 的角色/审查治理、Superpowers 的工程纪律融合。
