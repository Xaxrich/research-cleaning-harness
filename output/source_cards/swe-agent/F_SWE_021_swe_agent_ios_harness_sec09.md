# Source Card: F_SWE_021 - 9. Trajectory / Replay 深挖与 iOS 迁移
...
| source_id | F_SWE_021 |
| framework | swe-agent |
| raw_path | raw/Kimi_Agent_SWE-agent 迁移研究/swe_agent_ios_harness_sec09.md |
| file_type | markdown |
| topic | 9. Trajectory / Replay 深挖与 iOS 迁移 |
...
- 它围绕 `9. Trajectory / Replay 深挖与 iOS 迁移` 展开，主要贡献 ACI、工具、环境反馈、trajectory、workflow 或安全边界。
...
|---|---|---|---|---|
| M-SWE-081 | Trajectory Log Format | 记录 task metadata、thought/action/observation、state snapshot、termination 和 escalation。 | E1 | high |
| M-SWE-082 | Replay for Debugging | 用 trajectory 重放定位 stuck loop、错误命令和越界编辑。 | E2 | high |
| M-SWE-083 | Config and Trajectory Coupling | 把模型、工具、环境配置写入轨迹，保证结果可解释。 | E3 | high |
| M-SWE-084 | Privacy-Aware Logging | trajectory 不能无边界记录 secrets、隐私数据或完整源码。 | E4 | high |
...
|---|---|---|---|
| E1 | 主标题/首个 heading 指向：9. Trajectory / Replay 深挖与 iOS 迁移。 | raw/Kimi_Agent_SWE-agent 迁移研究/swe_agent_ios_harness_sec09.md:1 | source topic |
| E2 | 关键结构摘要：9. Trajectory / Replay 深挖与 iOS 迁移; 9.1 Trajectory 设计原理; 9.1.1 Trajectory 定义：结构化记录 agent 完整行为历史的数据结构; 9.1.2 与普通聊天记录的区别：包含 state、execution_time、tool_calls 等技术字段; 9.1.3 为什么 agent 行为必须可追踪：调试、失败恢复、review、迭代优化; 9.1.4 Trajectory 如何避免污染后续上下文：history_processors 压缩旧记录。 | raw/Kimi_Agent_SWE-agent 迁移研究/swe_agent_ios_harness_sec09.md:structure | mechanism structure |
| E3 | inventory 主题为：9. Trajectory / Replay 深挖与 iOS 迁移。 | SOURCE_INVENTORY.md | estimated topic |
| E4 | 文件类别 `markdown`，度量值 935，细节：markdown headings。 | raw/Kimi_Agent_SWE-agent 迁移研究/swe_agent_ios_harness_sec09.md:full file | scope and density |
| E5 | 该文件归类为 SWE-agent `trajectory` 清洗资料。 | raw/Kimi_Agent_SWE-agent 迁移研究/swe_agent_ios_harness_sec09.md:path | framework category |
...
|---|---|---|---|---|
| M-SWE-081 | Memory / State Layer | output/data/trajectory.jsonl | v0_5 | 建立 iOS Harness 可回放日志。 |
| M-SWE-082 | Feedback / Verification Layer | scripts/agent/replay_trajectory.sh | v1_0 | 后续实现回放工具。 |
| M-SWE-083 | Memory / State Layer | CONFIG.md | v0_5 | 将 config hash 或参数写入日志。 |
| M-SWE-084 | Risk / Release Layer | PRIVACY_CHECKLIST.md | v0_1 | 加入 redaction 和敏感文件规则。 |
...
这张卡把 `9. Trajectory / Replay 深挖与 iOS 迁移` 从原始 SWE-agent 调研文件转成可被 Codex 消费的 clean asset。后续 iOS Harness 构建时，不应直接读取 raw 文件理解 SWE-agent，而应读取这张 Source Card 及其 framework summary。它的主要价值是把 SWE-agent 的 Agent-Computer Interface、受控工具调用、环境反馈、trajectory/replay、命令验证、安全权限和 iOS 场景 workflow 迁移到 Harness 的 Action、Feedback、Memory 和 Risk 层。合成阶段需要与 Aider 的 repo/file scope、GSD2 的状态机与验证闭环、gstack 的角色/审查治理、Superpowers 的工程纪律融合。
