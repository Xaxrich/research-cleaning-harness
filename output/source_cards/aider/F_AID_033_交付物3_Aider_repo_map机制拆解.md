# Source Card: F_AID_033 - Aider repo map 机制拆解
...
| source_id | F_AID_033 |
| framework | aider |
| raw_path | raw/Kimi_Agent_Aider 代码库方案/交付物3_Aider_repo_map机制拆解.md |
| file_type | markdown |
| topic | Aider repo map 机制拆解 |
...
- 它围绕 `Aider repo map 机制拆解` 展开，提供 Aider 到 iOS Harness 的可迁移机制。
...
|---|---|---|---|---|
| M-AID-129 | Repo Map Ranking | 用符号、路径和引用关系为模型选择最相关的代码片段。 | E1 | high |
| M-AID-130 | Token-Budgeted Repository Context | 在 token 预算内压缩仓库结构和关键符号。 | E2 | high |
| M-AID-131 | Dependency Surfacing | 通过 repo map 暴露跨文件依赖，减少漏改关联文件。 | E3 | high |
| M-AID-132 | Repo Map Boundary | repo map 只能帮助发现相关文件，不能替代真实编译、测试或平台判断。 | E4 | high |
...
|---|---|---|---|
| E1 | 主标题/首个 heading 指向：Aider repo map 机制拆解。 | raw/Kimi_Agent_Aider 代码库方案/交付物3_Aider_repo_map机制拆解.md:1 | source topic |
| E2 | 关键章节包括：Aider repo map 机制拆解; 一、Repo Map 完整数据流; 1.1 数据流总览; 1.2 核心调用链; 二、Tree-sitter 符号提取; 2.1 Tree-sitter 工作原理。 | raw/Kimi_Agent_Aider 代码库方案/交付物3_Aider_repo_map机制拆解.md:headings | mechanism structure |
| E3 | inventory 主题为：Aider repo map 机制拆解。 | SOURCE_INVENTORY.md | estimated topic |
| E4 | 文件约 455 行，属于 Aider repo_map 资料，结构足以生成独立 Source Card。 | raw/Kimi_Agent_Aider 代码库方案/交付物3_Aider_repo_map机制拆解.md:full file | scope and density |
| E5 | 文件路径为 raw/Kimi_Agent_Aider 代码库方案/交付物3_Aider_repo_map机制拆解.md，归属 Aider raw 目录。 | raw/Kimi_Agent_Aider 代码库方案/交付物3_Aider_repo_map机制拆解.md:path | metadata |
...
|---|---|---|---|---|
| M-AID-129 | Context Layer | docs/agent/REPO_MAP.md | v0_5 | 为 iOS repo context 生成 ranked map。 |
| M-AID-130 | Context Layer | CONTEXT_INDEX.md | v0_5 | 把 repo map 作为可裁剪上下文，而非全仓读取。 |
| M-AID-131 | Task Layer | TASKS.md | v0_5 | 在任务卡加入 related_files 候选。 |
| M-AID-132 | Risk / Release Layer | DECISIONS.md | v0_1 | 在合成时记录 repo map 的非转移边界。 |
...
这张卡把 `Aider repo map 机制拆解` 从原始研究文件转成可被 Codex 消费的 Aider clean asset。后续 iOS Harness 构建时，不应直接读取 raw 文件来理解 Aider，而应读取这张 Source Card 及其 framework summary。它的主要价值是把 Aider 的 repo-aware editing、文件范围控制、只读上下文、Git 边界、验证闭环或弱模型限制迁移到 iOS Harness 的上下文层、任务层、验证层和风险层。合成阶段需要与 GSD2 的 context/state 机制、Superpowers 的工程纪律，以及后续 gstack/SWE-agent 的 tool/runtime 机制去重融合。
