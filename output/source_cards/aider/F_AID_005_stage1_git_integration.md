# Source Card: F_AID_005 - Aider Git 集成机制深度分析
...
| source_id | F_AID_005 |
| framework | aider |
| raw_path | raw/Kimi_Agent_Aider 代码库方案/stage1_git_integration.md |
| file_type | markdown |
| topic | Aider Git 集成机制深度分析 |
...
- 它围绕 `Aider Git 集成机制深度分析` 展开，提供 Aider 到 iOS Harness 的可迁移机制。
...
|---|---|---|---|---|
| M-AID-017 | Dirty Tree Awareness | 执行前识别已有未提交变更，避免覆盖用户工作。 | E1 | high |
| M-AID-018 | Atomic Commit Boundary | 单任务单 commit，让变更可审查、回滚和归因。 | E2 | high |
| M-AID-019 | Auto-Commit With Undo Semantics | 自动提交必须配套 undo/rollback 策略。 | E3 | high |
| M-AID-020 | PR/Commit Message Evidence | 提交和 PR 描述应包含范围、验证、风险和回滚说明。 | E4 | high |
...
|---|---|---|---|
| E1 | 主标题/首个 heading 指向：Aider Git 集成机制深度分析。 | raw/Kimi_Agent_Aider 代码库方案/stage1_git_integration.md:1 | source topic |
| E2 | 关键章节包括：Aider Git 集成机制深度分析; 一、核心机制概览; 二、问题逐一解答; 1. 为什么 AI coding 必须和 Git 强绑定？; 2. auto-commit 解决什么问题？触发条件是什么？; 解决的问题。 | raw/Kimi_Agent_Aider 代码库方案/stage1_git_integration.md:headings | mechanism structure |
| E3 | inventory 主题为：Aider Git 集成机制深度分析。 | SOURCE_INVENTORY.md | estimated topic |
| E4 | 文件约 322 行，属于 Aider git 资料，结构足以生成独立 Source Card。 | raw/Kimi_Agent_Aider 代码库方案/stage1_git_integration.md:full file | scope and density |
| E5 | 文件路径为 raw/Kimi_Agent_Aider 代码库方案/stage1_git_integration.md，归属 Aider raw 目录。 | raw/Kimi_Agent_Aider 代码库方案/stage1_git_integration.md:path | metadata |
...
|---|---|---|---|---|
| M-AID-017 | Risk / Release Layer | GIT_WORKFLOW.md | v0_1 | 要求任务前记录 dirty state。 |
| M-AID-018 | Memory / State Layer | TASKS.md | v0_1 | 任务完成记录 commit id 或 diff summary。 |
| M-AID-019 | Risk / Release Layer | GIT_ATOMIC_COMMIT.md | v0_5 | 把自动提交限制在验证通过后。 |
| M-AID-020 | Feedback / Verification Layer | templates/pr_description.md | v0_1 | 把审查证据写入提交/PR 模板。 |
