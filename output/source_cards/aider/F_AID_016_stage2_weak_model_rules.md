# Source Card: F_AID_016 - iOS Harness 弱模型执行规则体系
...
| source_id | F_AID_016 |
| framework | aider |
| raw_path | raw/Kimi_Agent_Aider 代码库方案/stage2_weak_model_rules.md |
| file_type | markdown |
...
|---|---|---|---|---|
| M-AID-061 | Weak Model File Cap | 弱模型只处理少量明确文件，避免跨层迷路。 | E1 | high |
| M-AID-062 | Weak Model Task Downgrade | 把复杂任务拆小或升级给强模型。 | E2 | high |
| M-AID-063 | Simplified Context Pack | 弱模型只接收目标文件、规则摘要和最小验证命令。 | E3 | high |
| M-AID-064 | Escalation on Scope Breach | 弱模型触碰禁止文件、失败循环或范围扩大时升级。 | E4 | high |
...
|---|---|---|---|
| E1 | 主标题/首个 heading 指向：iOS Harness 弱模型执行规则体系。 | raw/Kimi_Agent_Aider 代码库方案/stage2_weak_model_rules.md:1 | source topic |
| E2 | 关键章节包括：iOS Harness 弱模型执行规则体系; 目录; 一、弱模型任务类型限制表; 1.1 限制表主体; 1.2 关键定义; 1.3 风险评估维度。 | raw/Kimi_Agent_Aider 代码库方案/stage2_weak_model_rules.md:headings | mechanism structure |
| E3 | inventory 主题为：iOS Harness 弱模型执行规则体系。 | SOURCE_INVENTORY.md | estimated topic |
| E4 | 文件约 984 行，属于 Aider weak_model 资料，结构足以生成独立 Source Card。 | raw/Kimi_Agent_Aider 代码库方案/stage2_weak_model_rules.md:full file | scope and density |
| E5 | 文件路径为 raw/Kimi_Agent_Aider 代码库方案/stage2_weak_model_rules.md，归属 Aider raw 目录。 | raw/Kimi_Agent_Aider 代码库方案/stage2_weak_model_rules.md:path | metadata |
...
|---|---|---|---|---|
| M-AID-061 | Task Layer | TASKS.md | v0_1 | 为 weak_model task 设置 max_files。 |
| M-AID-062 | Role / Review Layer | MODEL_ROUTING.md | v0_5 | 将弱模型边界接入模型路由。 |
| M-AID-063 | Context Layer | CONTEXT_INDEX.md | v0_5 | 生成 weak context pack。 |
| M-AID-064 | Risk / Release Layer | FAILURE_LOG.md | v0_1 | 记录 scope_breach 并阻断继续执行。 |
