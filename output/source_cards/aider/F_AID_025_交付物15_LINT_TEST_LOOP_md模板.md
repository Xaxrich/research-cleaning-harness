# Source Card: F_AID_025 - iOS Harness 验证闭环设计文档
...
| source_id | F_AID_025 |
| framework | aider |
| raw_path | raw/Kimi_Agent_Aider 代码库方案/交付物15_LINT_TEST_LOOP.md模板.md |
| file_type | markdown |
...
|---|---|---|---|---|
| M-AID-097 | Lint Command Gate | 用 lint/analyze 命令发现静态错误。 | E1 | high |
| M-AID-098 | Test Command Gate | 用测试命令验证行为变更。 | E2 | high |
| M-AID-099 | Reflection Fix Loop | 命令失败后让模型读取错误、修复并重跑，但次数受限。 | E3 | high |
| M-AID-100 | Verification Evidence Record | 完成声明必须附命令、退出码和关键输出摘要。 | E4 | high |
...
|---|---|---|---|
| E1 | 主标题/首个 heading 指向：iOS Harness 验证闭环设计文档。 | raw/Kimi_Agent_Aider 代码库方案/交付物15_LINT_TEST_LOOP.md模板.md:14 | source topic |
| E2 | 关键章节包括：iOS Harness 验证闭环设计文档; 文档信息; 一、验证闭环的目的; 1.1 为什么需要验证闭环; 1.2 验证闭环的核心机制; 1.3 基本原则。 | raw/Kimi_Agent_Aider 代码库方案/交付物15_LINT_TEST_LOOP.md模板.md:headings | mechanism structure |
| E3 | inventory 主题为：iOS Harness 验证闭环设计文档。 | SOURCE_INVENTORY.md | estimated topic |
| E4 | 文件约 1246 行，属于 Aider verification 资料，结构足以生成独立 Source Card。 | raw/Kimi_Agent_Aider 代码库方案/交付物15_LINT_TEST_LOOP.md模板.md:full file | scope and density |
| E5 | 文件路径为 raw/Kimi_Agent_Aider 代码库方案/交付物15_LINT_TEST_LOOP.md模板.md，归属 Aider raw 目录。 | raw/Kimi_Agent_Aider 代码库方案/交付物15_LINT_TEST_LOOP.md模板.md:path | metadata |
...
|---|---|---|---|---|
| M-AID-097 | Feedback / Verification Layer | VERIFICATION_MATRIX.md | v0_1 | 把 `flutter analyze` 等命令列为完成前证据。 |
| M-AID-098 | Feedback / Verification Layer | VERIFICATION_MATRIX.md | v0_1 | 将 unit/widget/integration test 绑定任务类型。 |
| M-AID-099 | Feedback / Verification Layer | FAILURE_LOG.md | v0_5 | 记录失败命令、修复尝试和 retry_count。 |
| M-AID-100 | Memory / State Layer | output/data/verification_evidence.jsonl | v0_5 | 结构化保存验证证据。 |
