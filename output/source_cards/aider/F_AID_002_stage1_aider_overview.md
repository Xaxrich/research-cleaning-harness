# Source Card: F_AID_002 - Aider 底层设计问题与框架对比 — 阶段一研究报告
...
| source_id | F_AID_002 |
| framework | aider |
| raw_path | raw/Kimi_Agent_Aider 代码库方案/stage1_aider_overview.md |
| file_type | markdown |
| topic | Aider 底层设计问题与框架对比 — 阶段一研究报告 |
...
- 它围绕 `Aider 底层设计问题与框架对比 — 阶段一研究报告` 展开，提供 Aider 到 iOS Harness 的可迁移机制。
...
|---|---|---|---|---|
| M-AID-005 | Repo-Aware Editing Loop | Aider 的核心是围绕真实代码库进行对话式编辑，而不是孤立生成代码。 | E1 | high |
| M-AID-006 | Explicit Added Files | 模型只应编辑明确加入任务上下文的文件。 | E2 | high |
| M-AID-007 | Read-Only Rule Context | 规范、索引和架构文件应作为只读上下文加载。 | E3 | high |
| M-AID-008 | Edit Format Discipline | 通过可解析的 edit format 限制模型输出，降低错误补丁风险。 | E4 | high |
...
|---|---|---|---|
| E1 | 主标题/首个 heading 指向：Aider 底层设计问题与框架对比 — 阶段一研究报告。 | raw/Kimi_Agent_Aider 代码库方案/stage1_aider_overview.md:1 | source topic |
| E2 | 关键章节包括：Aider 底层设计问题与框架对比 — 阶段一研究报告; 第一部分：Aider 底层问题分析（10 个问题）; 问题 1：Aider 试图解决 coding agent 的什么底层问题？; 问题 2：Aider 与 Superpowers 的本质区别; 问题 3：Aider 与 GSD2 的本质区别; 问题 4：Aider 与 GStack 的本质区别。 | raw/Kimi_Agent_Aider 代码库方案/stage1_aider_overview.md:headings | mechanism structure |
| E3 | inventory 主题为：Aider 底层设计问题与框架对比 — 阶段一研究报告。 | SOURCE_INVENTORY.md | estimated topic |
| E4 | 文件约 308 行，属于 Aider aider_core 资料，结构足以生成独立 Source Card。 | raw/Kimi_Agent_Aider 代码库方案/stage1_aider_overview.md:full file | scope and density |
| E5 | 文件路径为 raw/Kimi_Agent_Aider 代码库方案/stage1_aider_overview.md，归属 Aider raw 目录。 | raw/Kimi_Agent_Aider 代码库方案/stage1_aider_overview.md:path | metadata |
...
|---|---|---|---|---|
| M-AID-005 | Context Layer | docs/agent/REPO_CONTEXT.md | v0_1 | 把 repo-aware 作为 iOS Harness 上下文层原则。 |
| M-AID-006 | Task Layer | TASKS.md | v0_1 | 在任务卡中固定 editable files。 |
| M-AID-007 | Context Layer | CONTEXT_INDEX.md | v0_1 | 区分 read-only context 与 editable files。 |
| M-AID-008 | Action / ACI Layer | docs/agent/EDIT_FORMATS.md | v0_5 | 将全文件、diff、udiff 等编辑格式按任务风险选择。 |
