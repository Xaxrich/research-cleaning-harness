# Source Card: F_AID_006 - Aider Lint/Test 验证闭环机制深度分析
...
| source_id | F_AID_006 |
| framework | aider |
| raw_path | raw/Kimi_Agent_Aider 代码库方案/stage1_lint_test_loop.md |
| file_type | markdown |
| topic | Aider Lint/Test 验证闭环机制深度分析 |
...
- 它围绕 `Aider Lint/Test 验证闭环机制深度分析` 展开，提供 Aider 到 iOS Harness 的可迁移机制。
...
|---|---|---|---|---|
| M-AID-021 | Lint Command Gate | 用 lint/analyze 命令发现静态错误。 | E1 | high |
| M-AID-022 | Test Command Gate | 用测试命令验证行为变更。 | E2 | high |
| M-AID-023 | Reflection Fix Loop | 命令失败后让模型读取错误、修复并重跑，但次数受限。 | E3 | high |
| M-AID-024 | Verification Evidence Record | 完成声明必须附命令、退出码和关键输出摘要。 | E4 | high |
...
|---|---|---|---|
| E1 | 主标题/首个 heading 指向：Aider Lint/Test 验证闭环机制深度分析。 | raw/Kimi_Agent_Aider 代码库方案/stage1_lint_test_loop.md:1 | source topic |
| E2 | 关键章节包括：Aider Lint/Test 验证闭环机制深度分析; 研究范围; 一、核心问题回答; 1. 为什么 coding agent 不能只靠自然语言判断完成？; 2. Aider 如何运行 lint / test？配置方式？; Lint 运行机制。 | raw/Kimi_Agent_Aider 代码库方案/stage1_lint_test_loop.md:headings | mechanism structure |
| E3 | inventory 主题为：Aider Lint/Test 验证闭环机制深度分析。 | SOURCE_INVENTORY.md | estimated topic |
| E4 | 文件约 695 行，属于 Aider verification 资料，结构足以生成独立 Source Card。 | raw/Kimi_Agent_Aider 代码库方案/stage1_lint_test_loop.md:full file | scope and density |
| E5 | 文件路径为 raw/Kimi_Agent_Aider 代码库方案/stage1_lint_test_loop.md，归属 Aider raw 目录。 | raw/Kimi_Agent_Aider 代码库方案/stage1_lint_test_loop.md:path | metadata |
...
|---|---|---|---|---|
| M-AID-021 | Feedback / Verification Layer | VERIFICATION_MATRIX.md | v0_1 | 把 `flutter analyze` 等命令列为完成前证据。 |
| M-AID-022 | Feedback / Verification Layer | VERIFICATION_MATRIX.md | v0_1 | 将 unit/widget/integration test 绑定任务类型。 |
| M-AID-023 | Feedback / Verification Layer | FAILURE_LOG.md | v0_5 | 记录失败命令、修复尝试和 retry_count。 |
| M-AID-024 | Memory / State Layer | output/data/verification_evidence.jsonl | v0_5 | 结构化保存验证证据。 |
