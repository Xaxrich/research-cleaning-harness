# Source Card: F_AID_010 - Aider 核心概念深度分析报告（Stage 2）
...
| source_id | F_AID_010 |
| framework | aider |
| raw_path | raw/Kimi_Agent_Aider 代码库方案/stage2_concepts_analysis.md |
| file_type | markdown |
| topic | Aider 核心概念深度分析报告（Stage 2） |
...
- 它围绕 `Aider 核心概念深度分析报告（Stage 2）` 展开，提供 Aider 到 iOS Harness 的可迁移机制。
...
|---|---|---|---|---|
| M-AID-037 | Repo-Aware Editing Loop | Aider 的核心是围绕真实代码库进行对话式编辑，而不是孤立生成代码。 | E1 | high |
| M-AID-038 | Explicit Added Files | 模型只应编辑明确加入任务上下文的文件。 | E2 | high |
| M-AID-039 | Read-Only Rule Context | 规范、索引和架构文件应作为只读上下文加载。 | E3 | high |
| M-AID-040 | Edit Format Discipline | 通过可解析的 edit format 限制模型输出，降低错误补丁风险。 | E4 | high |
...
|---|---|---|---|
| E1 | 主标题/首个 heading 指向：Aider 核心概念深度分析报告（Stage 2）。 | raw/Kimi_Agent_Aider 代码库方案/stage2_concepts_analysis.md:1 | source topic |
| E2 | 关键章节包括：Aider 核心概念深度分析报告（Stage 2）; Concept 1: repo map; 1. 一句话解释; 2. 它解决的问题; 3. 机制拆解; 4. 5 Why 分析。 | raw/Kimi_Agent_Aider 代码库方案/stage2_concepts_analysis.md:headings | mechanism structure |
| E3 | inventory 主题为：Aider 核心概念深度分析报告（Stage 2）。 | SOURCE_INVENTORY.md | estimated topic |
| E4 | 文件约 2033 行，属于 Aider aider_core 资料，结构足以生成独立 Source Card。 | raw/Kimi_Agent_Aider 代码库方案/stage2_concepts_analysis.md:full file | scope and density |
| E5 | 文件路径为 raw/Kimi_Agent_Aider 代码库方案/stage2_concepts_analysis.md，归属 Aider raw 目录。 | raw/Kimi_Agent_Aider 代码库方案/stage2_concepts_analysis.md:path | metadata |
...
|---|---|---|---|---|
| M-AID-037 | Context Layer | docs/agent/REPO_CONTEXT.md | v0_1 | 把 repo-aware 作为 iOS Harness 上下文层原则。 |
| M-AID-038 | Task Layer | TASKS.md | v0_1 | 在任务卡中固定 editable files。 |
| M-AID-039 | Context Layer | CONTEXT_INDEX.md | v0_1 | 区分 read-only context 与 editable files。 |
| M-AID-040 | Action / ACI Layer | docs/agent/EDIT_FORMATS.md | v0_5 | 将全文件、diff、udiff 等编辑格式按任务风险选择。 |
...
这张卡把 `Aider 核心概念深度分析报告（Stage 2）` 从原始研究文件转成可被 Codex 消费的 Aider clean asset。后续 iOS Harness 构建时，不应直接读取 raw 文件来理解 Aider，而应读取这张 Source Card 及其 framework summary。它的主要价值是把 Aider 的 repo-aware editing、文件范围控制、只读上下文、Git 边界、验证闭环或弱模型限制迁移到 iOS Harness 的上下文层、任务层、验证层和风险层。合成阶段需要与 GSD2 的 context/state 机制、Superpowers 的工程纪律，以及后续 gstack/SWE-agent 的 tool/runtime 机制去重融合。
