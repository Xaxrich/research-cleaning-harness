# Source Card: F_AID_036 - Superpowers + GSD2 + gstack + Aider 组合方案
...
| file_type | markdown |
| topic | Superpowers + GSD2 + gstack + Aider 组合方案 |
...
- 它围绕 `Superpowers + GSD2 + gstack + Aider 组合方案` 展开，提供 Aider 到 iOS Harness 的可迁移机制。
...
|---|---|---|---|---|
| M-AID-141 | Aider Context Layer Transfer | 将 Aider 的 repo map、file scope 和 read-only context 转成 iOS Harness context layer。 | E1 | high |
| M-AID-142 | Versioned iOS Harness File Tree | 用 v0.1/v0.5/v1.0 区分文件结构和自动化程度。 | E2 | high |
| M-AID-143 | Four-Framework Role Composition | 将 Aider 与 Superpowers、GSD2、gstack 做职责分离。 | E3 | high |
| M-AID-144 | Migration Handoff Package | 把上下文索引、文件范围、Git、验证和模板打包给后续 Codex。 | E4 | high |
...
|---|---|---|---|
| E1 | 主标题/首个 heading 指向：Superpowers + GSD2 + gstack + Aider 组合方案。 | raw/Kimi_Agent_Aider 代码库方案/交付物6_四框架组合方案.md:1 | source topic |
| E2 | 关键章节包括：Superpowers + GSD2 + gstack + Aider 组合方案; 一、四框架的分层定位图; 1.1 框架概览; 1.2 四框架在 AI Coding 工作流中的位置; 1.3 各层之间的关系; 二、重叠点和冲突点分析。 | raw/Kimi_Agent_Aider 代码库方案/交付物6_四框架组合方案.md:headings | mechanism structure |
| E3 | inventory 主题为：Superpowers + GSD2 + gstack + Aider 组合方案。 | SOURCE_INVENTORY.md | estimated topic |
| E4 | 文件约 604 行，属于 Aider migration 资料，结构足以生成独立 Source Card。 | raw/Kimi_Agent_Aider 代码库方案/交付物6_四框架组合方案.md:full file | scope and density |
| E5 | 文件路径为 raw/Kimi_Agent_Aider 代码库方案/交付物6_四框架组合方案.md，归属 Aider raw 目录。 | raw/Kimi_Agent_Aider 代码库方案/交付物6_四框架组合方案.md:path | metadata |
...
|---|---|---|---|---|
| M-AID-141 | Context Layer | CONTEXT_INDEX.md | v0_1 | 作为 Aider 迁移主线。 |
| M-AID-142 | Harness Maintenance Layer | output/ios_harness_mapping/v0_1_scope.md | v0_1 | 把迁移落到版本化文件树。 |
| M-AID-143 | Role / Review Layer | output/frameworks/aider_summary.md | v0_5 | 合成时保留职责边界。 |
| M-AID-144 | Harness Maintenance Layer | output/ios_harness_mapping/codex_handoff.md | v0_1 | 形成 Aider clean handoff。 |
...
这张卡把 `Superpowers + GSD2 + gstack + Aider 组合方案` 从原始研究文件转成可被 Codex 消费的 Aider clean asset。后续 iOS Harness 构建时，不应直接读取 raw 文件来理解 Aider，而应读取这张 Source Card 及其 framework summary。它的主要价值是把 Aider 的 repo-aware editing、文件范围控制、只读上下文、Git 边界、验证闭环或弱模型限制迁移到 iOS Harness 的上下文层、任务层、验证层和风险层。合成阶段需要与 GSD2 的 context/state 机制、Superpowers 的工程纪律，以及后续 gstack/SWE-agent 的 tool/runtime 机制去重融合。
