# Source Card: F_GST_006 - Role: Orchestrator（总协调者）
...
| source_id | F_GST_006 |
| framework | gstack |
| raw_path | raw/Kimi_Agent_gstack 多 Agent 迁移/ios-harness/.agents/roles/orchestrator.md |
| file_type | markdown |
| topic | Role: Orchestrator（总协调者） |
...
- 它围绕 `Role: Orchestrator（总协调者）` 展开，提供 agent 集群治理、角色、工作流、审查或 guardrail 相关机制。
...
|---|---|---|---|---|
| M-GST-021 | Specialized Agent Role Card | 为单个 iOS agent 角色定义职责、输入、输出和边界。 | E1 | high |
| M-GST-022 | Role-Specific File Ownership | 角色只处理自己负责的 Flutter/Firebase/Swift/QA/Release 范围。 | E2 | high |
| M-GST-023 | Role Handoff Contract | 角色间通过明确输入输出交接，避免职责混乱。 | E3 | high |
| M-GST-024 | Role Escalation Boundary | 超出职责时升级给 orchestrator 或高风险 reviewer。 | E4 | high |
...
|---|---|---|---|
| E1 | 主标题/首个 heading 指向：Role: Orchestrator（总协调者）。 | raw/Kimi_Agent_gstack 多 Agent 迁移/ios-harness/.agents/roles/orchestrator.md:1 | source topic |
| E2 | 关键结构摘要：Role: Orchestrator（总协调者）; 身份定义; 职责范围; 输入; 输出; 质量标准。 | raw/Kimi_Agent_gstack 多 Agent 迁移/ios-harness/.agents/roles/orchestrator.md:structure | mechanism structure |
| E3 | inventory 主题为：Role: Orchestrator（总协调者）。 | SOURCE_INVENTORY.md | estimated topic |
| E4 | 文件类别 `markdown`，度量值 233，细节：markdown headings。 | raw/Kimi_Agent_gstack 多 Agent 迁移/ios-harness/.agents/roles/orchestrator.md:full file | scope and density |
| E5 | 该文件归类为 gstack `role_card` 清洗资料。 | raw/Kimi_Agent_gstack 多 Agent 迁移/ios-harness/.agents/roles/orchestrator.md:path | framework category |
...
|---|---|---|---|---|
| M-GST-021 | Role / Review Layer | ROLE_MATRIX.md | v0_1 | 将角色卡汇总进角色矩阵。 |
| M-GST-022 | Task Layer | TASKS.md | v0_1 | 任务卡绑定 owner_role。 |
| M-GST-023 | Memory / State Layer | STATE.md | v0_5 | 记录 upstream/downstream role。 |
| M-GST-024 | Risk / Release Layer | RISK_CONTROL.md | v0_1 | 把 role boundary breach 作为风险事件。 |
...
这张卡把 `Role: Orchestrator（总协调者）` 从原始 gstack 研究或模板文件转成可被 Codex 消费的 clean asset。后续 iOS Harness 构建时，不应直接读取 raw 文件理解 gstack，而应读取这张 Source Card 及其 framework summary。它的主要价值是把 gstack 的虚拟工程团队、角色矩阵、审查链路、风险控制、工作流、技能路由或 guardrail 机制迁移到 iOS Harness 的 Role/Review、Risk/Release、Task、Action 和 Feedback 层。合成阶段需要与 Superpowers 的工程纪律、GSD2 的状态/上下文/验证机制、Aider 的 repo/file scope 机制以及后续 SWE-agent 的 tool/runtime 机制去重融合。
