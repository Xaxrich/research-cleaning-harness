# Source Card: F_GST_036 - gstack 仓库命令体系、安全机制与 Guardrails 深度分析报告
...
| source_id | F_GST_036 |
| framework | gstack |
| raw_path | raw/Kimi_Agent_gstack 多 Agent 迁移/research/gstack_commands.md |
| file_type | markdown |
| topic | gstack 仓库命令体系、安全机制与 Guardrails 深度分析报告 |
...
- 它围绕 `gstack 仓库命令体系、安全机制与 Guardrails 深度分析报告` 展开，提供 agent 集群治理、角色、工作流、审查或 guardrail 相关机制。
...
|---|---|---|---|---|
| M-GST-141 | Command Guardrail Mode | 用 guard/careful/freeze 类机制阻断危险命令和越界编辑。 | E1 | high |
| M-GST-142 | Directory Freeze | 冻结敏感目录或限定编辑目录。 | E2 | high |
| M-GST-143 | Risk Matrix for Commands | 按命令风险分级，决定 allow/ask/deny/review。 | E3 | high |
| M-GST-144 | Hook-Based Enforcement Boundary | hook 是未来执行层，当前清洗只保留机制和迁移目标。 | E4 | high |
...
|---|---|---|---|
| E1 | 主标题/首个 heading 指向：gstack 仓库命令体系、安全机制与 Guardrails 深度分析报告。 | raw/Kimi_Agent_gstack 多 Agent 迁移/research/gstack_commands.md:1 | source topic |
| E2 | 关键结构摘要：gstack 仓库命令体系、安全机制与 Guardrails 深度分析报告; 第一部分：命令总表; 第二部分：安全机制详解; 2.1 `/guard` — 完整安全模式 **[公开确认]**; 机制原理; 触发条件。 | raw/Kimi_Agent_gstack 多 Agent 迁移/research/gstack_commands.md:structure | mechanism structure |
| E3 | inventory 主题为：gstack 仓库命令体系、安全机制与 Guardrails 深度分析报告。 | SOURCE_INVENTORY.md | estimated topic |
| E4 | 文件类别 `markdown`，度量值 642，细节：markdown headings。 | raw/Kimi_Agent_gstack 多 Agent 迁移/research/gstack_commands.md:full file | scope and density |
| E5 | 该文件归类为 gstack `guardrails` 清洗资料。 | raw/Kimi_Agent_gstack 多 Agent 迁移/research/gstack_commands.md:path | framework category |
...
|---|---|---|---|---|
| M-GST-141 | Action / ACI Layer | scripts/agent/guardrails/ | v0_5 | 把风险规则脚本化。 |
| M-GST-142 | Risk / Release Layer | HIGH_RISK_FILES.md | v0_5 | 将 freeze 规则映射到 forbidden_files。 |
| M-GST-143 | Risk / Release Layer | RISK_CONTROL.md | v0_1 | 先用文档规则，后续脚本拦截。 |
| M-GST-144 | Harness Maintenance Layer | DECISIONS.md | v0_1 | 避免声称已实现 runtime hook。 |
...
这张卡把 `gstack 仓库命令体系、安全机制与 Guardrails 深度分析报告` 从原始 gstack 研究或模板文件转成可被 Codex 消费的 clean asset。后续 iOS Harness 构建时，不应直接读取 raw 文件理解 gstack，而应读取这张 Source Card 及其 framework summary。它的主要价值是把 gstack 的虚拟工程团队、角色矩阵、审查链路、风险控制、工作流、技能路由或 guardrail 机制迁移到 iOS Harness 的 Role/Review、Risk/Release、Task、Action 和 Feedback 层。合成阶段需要与 Superpowers 的工程纪律、GSD2 的状态/上下文/验证机制、Aider 的 repo/file scope 机制以及后续 SWE-agent 的 tool/runtime 机制去重融合。
