# Source Card: F_GST_031 - gstack development
...
| source_id | F_GST_031 |
| framework | gstack |
| raw_path | raw/Kimi_Agent_gstack 多 Agent 迁移/research/CLAUDE.md |
| file_type | markdown |
| topic | gstack development |
...
- 它围绕 `gstack development` 展开，提供 agent 集群治理、角色、工作流、审查或 guardrail 相关机制。
...
|---|---|---|---|---|
| M-GST-121 | Search Before Building | 先搜索现有知识和实现，再新增能力。 | E1 | high |
| M-GST-122 | User Sovereignty | 用户目标和约束优先于框架偏好。 | E2 | high |
| M-GST-123 | Long-Running Task Persistence | 长任务不要轻易放弃，需要计划、状态和恢复。 | E3 | high |
| M-GST-124 | Engineering Quality Over AI Style | 关注真实质量问题，不做表面化 AI 文风修饰。 | E4 | high |
...
|---|---|---|---|
| E1 | 主标题/首个 heading 指向：gstack development。 | raw/Kimi_Agent_gstack 多 Agent 迁移/research/CLAUDE.md:1 | source topic |
| E2 | 关键结构摘要：gstack development; Commands; Testing; Project structure; SKILL.md workflow; Platform-agnostic design。 | raw/Kimi_Agent_gstack 多 Agent 迁移/research/CLAUDE.md:structure | mechanism structure |
| E3 | inventory 主题为：gstack development。 | SOURCE_INVENTORY.md | estimated topic |
| E4 | 文件类别 `markdown`，度量值 817，细节：markdown headings。 | raw/Kimi_Agent_gstack 多 Agent 迁移/research/CLAUDE.md:full file | scope and density |
| E5 | 该文件归类为 gstack `ethos_core` 清洗资料。 | raw/Kimi_Agent_gstack 多 Agent 迁移/research/CLAUDE.md:path | framework category |
...
|---|---|---|---|---|
| M-GST-121 | Context Layer | CONTEXT_RULES.md | v0_1 | 要求任务前检索相关 source cards。 |
| M-GST-122 | Goal Layer | AGENTS.md | v0_1 | 写入 harness 执行原则。 |
| M-GST-123 | Memory / State Layer | STATE.md | v0_1 | 维护任务进度和阻塞。 |
| M-GST-124 | Feedback / Verification Layer | QUALITY_GATE.md | v0_1 | 把质量门聚焦到行为和证据。 |
...
这张卡把 `gstack development` 从原始 gstack 研究或模板文件转成可被 Codex 消费的 clean asset。后续 iOS Harness 构建时，不应直接读取 raw 文件理解 gstack，而应读取这张 Source Card 及其 framework summary。它的主要价值是把 gstack 的虚拟工程团队、角色矩阵、审查链路、风险控制、工作流、技能路由或 guardrail 机制迁移到 iOS Harness 的 Role/Review、Risk/Release、Task、Action 和 Feedback 层。合成阶段需要与 Superpowers 的工程纪律、GSD2 的状态/上下文/验证机制、Aider 的 repo/file scope 机制以及后续 SWE-agent 的 tool/runtime 机制去重融合。
