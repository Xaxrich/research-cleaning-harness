# Source Card: F_GST_011 - Firebase Rules 检查模板
...
| source_id | F_GST_011 |
| framework | gstack |
| raw_path | raw/Kimi_Agent_gstack 多 Agent 迁移/ios-harness/.continue/checks/firebase-rules.md |
| file_type | markdown |
| topic | Firebase Rules 检查模板 |
...
- 它围绕 `Firebase Rules 检查模板` 展开，提供 agent 集群治理、角色、工作流、审查或 guardrail 相关机制。
...
|---|---|---|---|---|
| M-GST-041 | Continue Check Template | 为高风险领域定义可重复的检查清单。 | E1 | high |
| M-GST-042 | Domain-Specific Gate | App Store、Firebase rules、安全等领域拥有专门 gate。 | E2 | high |
| M-GST-043 | Review Evidence Checklist | 检查模板要求记录证据而非只给结论。 | E3 | high |
| M-GST-044 | Reusable QA Surface | 将检查项模板化，供不同任务复用。 | E4 | high |
...
|---|---|---|---|
| E1 | 主标题/首个 heading 指向：Firebase Rules 检查模板。 | raw/Kimi_Agent_gstack 多 Agent 迁移/ios-harness/.continue/checks/firebase-rules.md:1 | source topic |
| E2 | 关键结构摘要：Firebase Rules 检查模板; 一、Rules 语法正确性; 1.1 基本语法检查; 1.2 最佳实践; 二、权限最小化; 2.1 默认拒绝原则。 | raw/Kimi_Agent_gstack 多 Agent 迁移/ios-harness/.continue/checks/firebase-rules.md:structure | mechanism structure |
| E3 | inventory 主题为：Firebase Rules 检查模板。 | SOURCE_INVENTORY.md | estimated topic |
| E4 | 文件类别 `markdown`，度量值 352，细节：markdown headings。 | raw/Kimi_Agent_gstack 多 Agent 迁移/ios-harness/.continue/checks/firebase-rules.md:full file | scope and density |
| E5 | 该文件归类为 gstack `check_template` 清洗资料。 | raw/Kimi_Agent_gstack 多 Agent 迁移/ios-harness/.continue/checks/firebase-rules.md:path | framework category |
...
|---|---|---|---|---|
| M-GST-041 | Feedback / Verification Layer | VERIFICATION_MATRIX.md | v0_1 | 将 check template 转成验证项。 |
| M-GST-042 | Risk / Release Layer | IOS_RELEASE_CHECKLIST.md | v0_1 | 把领域 gate 接入 release/risk 文件。 |
| M-GST-043 | Feedback / Verification Layer | output/data/verification_evidence.jsonl | v0_5 | 保存检查结果和证据摘要。 |
| M-GST-044 | Harness Maintenance Layer | templates/check_template.md | v0_5 | 抽象为可复用模板。 |
...
这张卡把 `Firebase Rules 检查模板` 从原始 gstack 研究或模板文件转成可被 Codex 消费的 clean asset。后续 iOS Harness 构建时，不应直接读取 raw 文件理解 gstack，而应读取这张 Source Card 及其 framework summary。它的主要价值是把 gstack 的虚拟工程团队、角色矩阵、审查链路、风险控制、工作流、技能路由或 guardrail 机制迁移到 iOS Harness 的 Role/Review、Risk/Release、Task、Action 和 Feedback 层。合成阶段需要与 Superpowers 的工程纪律、GSD2 的状态/上下文/验证机制、Aider 的 repo/file scope 机制以及后续 SWE-agent 的 tool/runtime 机制去重融合。
