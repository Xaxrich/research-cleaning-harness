# Source Card: F_GST_023 - iOS Harness 工作流 3：Firebase 安全规则流
...
| source_id | F_GST_023 |
| framework | gstack |
| raw_path | raw/Kimi_Agent_gstack 多 Agent 迁移/ios-harness/docs/workflow-firebase-rules.md |
| file_type | markdown |
| topic | iOS Harness 工作流 3：Firebase 安全规则流 |
...
- 它围绕 `iOS Harness 工作流 3：Firebase 安全规则流` 展开，提供 agent 集群治理、角色、工作流、审查或 guardrail 相关机制。
...
|---|---|---|---|---|
| M-GST-089 | Role-Oriented Workflow | 每类 iOS 工作流定义参与角色、输入、输出和步骤。 | E1 | high |
| M-GST-090 | Typed Workflow Gate | 新功能、bug、Firebase、Swift、发布等流程有不同 gate。 | E2 | high |
| M-GST-091 | Escalation Path per Workflow | 每个 workflow 规定失败升级和阻断条件。 | E3 | high |
| M-GST-092 | Completion Criteria Script | 工作流包含完成标准和可执行检查目标。 | E4 | high |
...
|---|---|---|---|
| E1 | 主标题/首个 heading 指向：iOS Harness 工作流 3：Firebase 安全规则流。 | raw/Kimi_Agent_gstack 多 Agent 迁移/ios-harness/docs/workflow-firebase-rules.md:1 | source topic |
| E2 | 关键结构摘要：iOS Harness 工作流 3：Firebase 安全规则流; 1. 适用场景; 2. 参与角色（列出所有角色及职责）; 3. 输入文件（具体文件路径）; 数据模型输入; 现有规则输入。 | raw/Kimi_Agent_gstack 多 Agent 迁移/ios-harness/docs/workflow-firebase-rules.md:structure | mechanism structure |
| E3 | inventory 主题为：iOS Harness 工作流 3：Firebase 安全规则流。 | SOURCE_INVENTORY.md | estimated topic |
| E4 | 文件类别 `markdown`，度量值 437，细节：markdown headings。 | raw/Kimi_Agent_gstack 多 Agent 迁移/ios-harness/docs/workflow-firebase-rules.md:full file | scope and density |
| E5 | 该文件归类为 gstack `workflow` 清洗资料。 | raw/Kimi_Agent_gstack 多 Agent 迁移/ios-harness/docs/workflow-firebase-rules.md:path | framework category |
...
|---|---|---|---|---|
| M-GST-089 | Task Layer | TASKS.md | v0_1 | 将 workflow 转为 task template。 |
| M-GST-090 | Feedback / Verification Layer | VERIFICATION_MATRIX.md | v0_1 | 按 workflow_type 选择验证项。 |
| M-GST-091 | Risk / Release Layer | RISK_CONTROL.md | v0_1 | 将升级路径写入风险控制。 |
| M-GST-092 | Action / ACI Layer | scripts/agent/workflow_checks/ | v0_5 | 后续脚本化工作流完成检查。 |
...
这张卡把 `iOS Harness 工作流 3：Firebase 安全规则流` 从原始 gstack 研究或模板文件转成可被 Codex 消费的 clean asset。后续 iOS Harness 构建时，不应直接读取 raw 文件理解 gstack，而应读取这张 Source Card 及其 framework summary。它的主要价值是把 gstack 的虚拟工程团队、角色矩阵、审查链路、风险控制、工作流、技能路由或 guardrail 机制迁移到 iOS Harness 的 Role/Review、Risk/Release、Task、Action 和 Feedback 层。合成阶段需要与 Superpowers 的工程纪律、GSD2 的状态/上下文/验证机制、Aider 的 repo/file scope 机制以及后续 SWE-agent 的 tool/runtime 机制去重融合。
